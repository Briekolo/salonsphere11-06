...
class SubscriptionService {
   ...
   * This creates an active paid subscription
   */
  async simulatePayment(tenantId: string, planId: string): Promise<Subscription> {
    if (process.env.NODE_ENV !== 'development') {
      throw new Error('Payment simulation only available in development mode')
    }
    ...
    if (existingSubscription) {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          trial_end: null,
          cancelled_at: null,
          mollie_subscription_id: `sim_subscription_${Date.now()}`
        })
      ...
      await supabase
        ...
        .insert({
          ...
          mollie_payment_id: `sim_payment_${Date.now()}`
        })
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          current_period_start: now.toISOString(),
          current_period_end: nextMonth.toISOString(),
          mollie_customer_id: `sim_customer_${tenantId}`,
          mollie_subscription_id: `sim_subscription_${Date.now()}`
        })
        .select()
        .single()
