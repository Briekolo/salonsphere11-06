...
export function MarketingDashboard({ onViewChange }: MarketingDashboardProps) {
  ...
  const { data: analytics, isLoading: analyticsLoading } = useCampaignAnalytics('month')
  const { data: segments, isLoading: segmentsLoading } = useCustomerSegments()

  // Transform segment data for the pie chart
  const segmentData = segments?.map((segment, index) => ({
    name: segment.name,
    value: segment.member_count || 0,
    color: ['#7091D9', '#ABD37A', '#A977FD', '#EBB474'][index % 4]
  })) || []
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              ) : analytics?.recentCampaigns?.length ? (
                  ...
                  <tr key={campaign.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4">
                      <span className="text-sm bg-primary-100 text-primary-800 px-2 py-1 rounded-full">
                        {campaign.status === 'sent' ? 'Verzonden' : 'Actief'}
                      </span>
                    </td>
                    <td className="py-4 px-4">
                      <span className={`status-chip ${
                        ...
                        {campaign.status === 'sent' ? 'Verzonden' : 
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="card text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-2">
            {analytics?.totalEmailsSent?.toLocaleString() || '0'}
          </h3>
          <p className="text-gray-600">E-mails verzonden deze maand</p>
          <div className="text-sm text-gray-500 mt-2">
            {analytics?.totalCampaigns || 0} campagnes
          </div>
        </div>
        ...
        <div className="card text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-2">
            {analytics?.averageOpenRate?.toFixed(1) || '0'}%
          </h3>
          <p className="text-gray-600">Gemiddelde open rate</p>
          <div className="text-sm text-blue-600 mt-2">
            {analytics?.averageClickRate?.toFixed(1) || '0'}% click rate
          </div>
        </div>
        ...
        <div className="card text-center">
          ...
          <p className="text-gray-600">Actieve campagnes</p>
          <div className="text-sm text-gray-500 mt-2">
            Lopend of ingepland
          </div>
        </div>
