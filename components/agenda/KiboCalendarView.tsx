...
import { MobileCalendarCell } from './MobileCalendarCell'
import { ChevronLeft, ChevronRight, Clock, User, MapPin, Plus, MoreHorizontal, GripVertical, Phone, Mail, Euro, Star, Users, Filter, X } from 'lucide-react'
...
// Mobile Day Detail View Component
function MobileDayDetailView({ date, bookings, onClose, onBookingClick, onNewBooking }: {
  date: Date
  bookings: Booking[]
  onClose: () => void
  onBookingClick: (bookingId: string) => void
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 md:hidden">
      <div className="absolute inset-x-0 bottom-0 bg-white rounded-t-3xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            <h3 className="text-lg font-semibold capitalize">{dayName}</h3>
            <p className={`text-sm ${isCurrentDay ? 'text-blue-600 font-medium' : 'text-gray-600'}`}>
              {dateStr}
            </p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {bookings.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500 mb-4">Geen afspraken op deze dag</p>
              <button
                onClick={() => onNewBooking(date)}
                className="px-4 py-2 bg-[#02011F] text-white rounded-lg hover:bg-opacity-90 transition-colors"
              >
            ...
            <div className="space-y-3">
              {bookings.map((booking) => {
                ...
                return (
                  <div
                    key={booking.id}
                    onClick={() => onBookingClick(booking.id)}
                    className={`p-4 rounded-lg border cursor-pointer transition-all ${colors.bg} ${colors.text}`}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full mt-1 ${colors.dot}`} />
                        <div>
                          <div className="font-medium">{clientName}</div>
                          <div className="text-sm opacity-80">{serviceName}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        {booking.clients?.phone && (
                          <div className="flex items-center gap-1 opacity-70">
                            <Phone className="w-3 h-3" />
                            <span className="text-xs">{booking.clients.phone}</span>
                          </div>
                        )}
                      </div>
                      {price > 0 && (
              ...
              >
                ...
                <span>Nieuwe afspraak toevoegen</span>
              </button>
            </div>
          )}
        </div>
...
export function KiboCalendarView({ selectedDate, onDateSelect, filters }: KiboCalendarViewProps) {
  ...
  return (
    <div className="h-full flex flex-col">
                ...
                {calendarDays.map((date) => {
                  ...
                  return isMobile ? (
                    <MobileCalendarCell
                      ...
                      onDayClick={(clickedDate) => setSelectedMobileDay(clickedDate)}
      ...
      {selectedMobileDay && isMobile && (
        <MobileDayDetailView
          date={selectedMobileDay}
          bookings={bookingsByDate[format(selectedMobileDay, 'yyyy-MM-dd')] || []}
          onClose={() => setSelectedMobileDay(null)}
          onBookingClick={(bookingId) => {
            setSelectedMobileDay(null)
            openModalForEdit(bookingId)
          }}
          onNewBooking={(date) => {
            setSelectedMobileDay(null)
